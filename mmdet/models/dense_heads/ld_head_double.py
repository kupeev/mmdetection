...
class LDHeadDouble(LDHead):
    ...
    def forward_train(self,
        ...
        if gt_labels is None:
            ...
            loss_inputs = outs + (gt_bboxes, gt_labels, soft_target, img_metas, soft_student)
    ...
    def loss(self,
             ...
             soft_target,
             img_metas,
             soft_student,#=outs_student[1][3]:  (1, 68, 13, 13)
             gt_bboxes_ignore=None):
        ...
        assert self.__class__.__name__ == 'LDHeadDouble'
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, label_weights,
                    bbox_targets, stride, soft_targets, soft_students, num_total_samples):
        """Compute loss of a single scale level.

        # qq inst bbox_pred calc soft_students
        ...
        soft_students = soft_students.permute(0, 2, 3,
                                      1).reshape(-1, 4 * (self.reg_max + 1))
        ...
        if len(pos_inds) > 0:
            ...
            pos_soft_students = soft_students[pos_inds]
            ...
            pos_soft_students_corners = self.integral(pos_soft_students)
            pos_decode_bbox_pred = self.bbox_coder.decode(
                pos_anchor_centers, pos_bbox_pred_corners)
            pos_decode_soft_students = self.bbox_coder.decode(
                pos_anchor_centers, pos_soft_students_corners)
            pos_decode_bbox_targets = pos_bbox_targets / stride[0]
            score[pos_inds] = bbox_overlaps(
                pos_decode_bbox_pred.detach(),
                pos_decode_soft_students,
                is_aligned=True)
            pred_corners = pos_bbox_pred.reshape(-1, self.reg_max + 1)
            pred_corners = pos_soft_students.reshape(-1, self.reg_max + 1)
